...
2.4 部署代码
# 复制完整的 _worker.js 代码到 src/worker.js
# 设置环境变量
...
# 密码数据
password_{userId}_{passwordId} = {加密密码数据}

# 分类数据
categories_{userId} = [分类列表]

# WebDAV 配置
webdav_config_{userId} = {加密WebDAV配置}

# OAuth 状态
oauth_state_{state} = "valid"
...
密码管理
获取密码列表
GET /api/passwords?page=1&limit=50&search={query}&category={category}
Authorization: Bearer {token}
...
{
  "webdavUrl": "https://webdav.teracloud.jp/dav/",
...
{
  "filename": "password-backup-2024-01-01.json",
  "restorePassword": "backup123"
}
...
# 验证回调地址
# 确保 OAUTH_REDIRECT_URI 与 OAuth 应用配置一致

# 检查日志
wrangler tail

2. 扩展无法填充
症状：Tampermonkey 扩展检测到账户但填充失败
解决方案：
...
检查 API 返回的分页信息
确认 renderPagination 函数正确执行
验证 CSS 样式是否加载

调试工具
扩展调试
// Tampermonkey 菜单 → 调试信息
// 查看控制台输出的详细信息
...
curl -H "Authorization: Bearer {token}" \
     https://your-domain.pages.dev/api/passwords
